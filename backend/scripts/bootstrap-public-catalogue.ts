...
const explore = new Set(["lake-tana", "blue-nile-falls", "lalibela", "yemrehanna-kristos", "axum", "yeha"]);
const places = [...new Map([...simienPlaces, ...gondarPlaces].map(p => [p.slug, p])).values()];
const destinations = places.map((p, i) => {
  const area = simienSlugs.has(p.slug) || p.slug === "highland-villages" ? "simien" : explore.has(p.slug) ? "explore" : "gondar";
  ...
  return { ...p, area, type: "other", alsoKnownAs: "alsoKnownAs" in p ? p.alsoKnownAs : [],
    ...
    path: `/${area === "simien" ? "simien-mountains" : area === "explore" ? "explore-ethiopia" : "gondar"}/${p.slug}`, locale: "en", availableLocales: ["en"], updatedAt };
});
