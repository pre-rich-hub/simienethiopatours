...
export default function GuidePage() {
  return <PageShell>
    ...
    <section className="section"><div className="shell notes-grid">{notes.map((note, index) => <article key={note.title}><span>0{index+1} · {note.tag}</span><h2>{note.title}</h2><p>{note.body}</p></article>)}</div></section>
