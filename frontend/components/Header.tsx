...
import { getTours, getDestinations, tourMedia, resolveMediaUrl } from "@/lib/catalogue";
...
export async function Header({ light = false }: { light?: boolean }) {
  ...
  const tourCards = tours.map(row => { const media = tourMedia(row); return { slug: row.slug, href: row.path, locale: row.locale, tag: row.duration ?? "", title: row.tourName, style: row.style ?? "", summary: cardBlurb(row.summary ?? "", 110), image: media.image, imageAlt: media.imageAlt }; });
