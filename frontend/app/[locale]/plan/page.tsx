...
export const metadata = pageMetadata({
  title: "Plan Your Simien Journey",
  description: "Tell Tevan your timing, walking comfort and interests to receive a locally planned Gondar and Simien journey.",
  path: "/plan",
});
...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ experience?: string; journey?: string }> }) {
  ...
  return <PageShell>
    <section className="page-hero plan-hero"><div className="shell plan-hero__grid"><div><div className="breadcrumbs"><Link href="/">Home</Link><span>/</span><span>Plan your journey</span></div><p className="eyebrow eyebrow--copper">No fixed script · no booking pressure</p><h1 className="display">You bring the curiosity. <em>We handle the complexity.</em></h1></div><p className="lead">{site.name}, operated by {site.legalOperator} from {site.address}, plans Gondar and Simien Mountains journeys with Tesema “Tevan” Mulualem. You do not need to know the route. Tell us your time, interests and the questions on your mind.</p></div></section>
    <section className="section plan-section"><div className="shell plan-layout"><div className="plan-sidebar"><p className="eyebrow">What happens next</p><ol><li><span>01</span><div><strong>You share the outline</strong><p>Dates, group, interests and how you like to travel.</p></div></li><li><span>02</span><div><strong>Tevan asks useful questions</strong><p>Walking comfort, altitude, camping and priorities.</p></div></li><li><span>03</span><div><strong>We shape the journey</strong><p>A route and quote grounded in current logistics.</p></div></li></ol><div className="direct-contact"><h2>Prefer to talk?</h2>
                <p>{site.name} · {site.legalOperator}<br />{site.address}</p><a href={site.whatsapp} target="_blank" rel="noreferrer"><MessageCircle />WhatsApp Tevan</a><a href={`tel:${site.phone}`}><Phone />{site.phoneDisplay}</a><a href={`mailto:${site.email}`}><Mail />{site.email}</a><p><Clock />Gondar is on East Africa Time (UTC+3).</p></div></div><div className="plan-form"><p className="eyebrow eyebrow--copper">Your journey outline</p><h2>Start where you are.</h2><InquiryForm key={selected} initialExperience={selected} /></div></div></section>
  </PageShell>;
}
