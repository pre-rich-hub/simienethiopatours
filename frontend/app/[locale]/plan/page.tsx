    ...
    <PageShell>
      ...
      <section className="page-hero plan-hero">
        <div className="shell plan-hero__grid">
          ...
          <p className="lead">{t("lead", { name: site.name, operator: site.legalOperator, address: site.address })}</p>
        </div>
      </section>
      <section className="section plan-section">
        <div className="shell plan-layout">
          <div className="plan-sidebar">
            ...
            <nav className="page-links" aria-label={t("guidesNav")}>
              <Link href="/simien-mountains/planning">{t("simienGuide")}</Link>
              <Link href="/gondar/planning">{t("gondarGuide")}</Link>
            </nav>
            <div className="direct-contact">
              <h2>{t("preferTalk")}</h2>
              <p>
                {site.name} · {site.legalOperator}
                <br />
                {site.address}
              </p>
          ...
          <div className="plan-form">
            <p className="eyebrow eyebrow--copper">{t("formTag")}</p>
            <h2>{t("formTitle")}</h2>
            <InquiryForm key={`${selected}:${message}`} initialExperience={selected} initialMessage={message} />
          </div>
        </div>
      </section>
    </PageShell>
